...
`@ying-companion/ai-core` 的 PostgreSQL + pgvector 长期记忆适配包。实现 `MemoryProvider` 与 `EmbeddingProvider`，将 Core 的记忆抽象落地为**可持久化、可语义检索**的向量存储。
...
> 完整 Core 编排见 [`packages/ai-core/README.md`](../ai-core/README.md) §3。
