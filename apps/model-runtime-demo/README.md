...
- `DATABASE_URL` 存在但 health 失败（连接 / pgvector / 表缺失）：严格使用 `NoopMemoryProvider`，面板显示 `error` 与具体原因，聊天仍可用但 recall/save 无效果。
...
health 状态由 `GET /api/memory-health` 在页面加载与每轮聊天后刷新，不在 chat 请求路径中做重型检测。
