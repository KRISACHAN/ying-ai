...
3. .agents/rules/         ← project rules (ai-guide, apps context)
4. docs/ai/platforms.md   ← tool map; then native assets for your IDE
...
**Rule precedence:** user prompt → nearest package `README.md` → this file → `docs/ai/core/` → `.agents/rules/` → native tool assets (`.codex/`, `.cursor/rules/`, `.claude/`, `.agents/skills/`). Details: [docs/ai/guidance-schema.md](docs/ai/guidance-schema.md) · [docs/ai/platforms.md](docs/ai/platforms.md).
...
| **Story Core SDK**  | `packages/story-core`                                         | **V1.3 complete** — Model Planner/Renderer, streamed Story Workflow, validated state timeline |
| **Story Postgres**  | `packages/story-postgres`                                     | **V1.3 complete** — Story Session / State / Turn / Message / Summary persistence              |
| **Debug app**       | `apps/model-runtime-demo`                                     | **V1.3 complete** — Model-backed Story Workbench, Story NDJSON adapter/debug UI               |
| **Story Architect** | `apps/story-architect`                                        | Independent authoring app — worldbuilding, characters, outline, and chapter drafting          |

> **Knowledge workspace** (desktop app + document-loader / data-organizer / data-searcher) moved to the sibling repo `ying-knowledge`.
...
| Write a new review                  | `.agents/skills/code-review/` or `.claude/skills/code-review/` → `.code-reviews/<app-name>/{n}-{slug}/` |

Full naming rules: [`.code-reviews/README.md`](.code-reviews/README.md). Follow-up skill: `.agents/skills/code-review-followup/` or `.claude/skills/code-review-followup/`.
...
| File                                                   | When to read                                                                      |
| ------------------------------------------------------ | --------------------------------------------------------------------------------- |
| [ai-guide.md](.agents/rules/ai-guide.md)               | Always — cross-tool entry after `docs/ai/core/`                                   |
| [project-context.md](.agents/rules/project-context.md) | When editing under `apps/**` (also injected via [apps/AGENTS.md](apps/AGENTS.md)) |

Tool adapters: [`.codex/rules/`](.codex/rules/) · [`.cursor/rules/`](.cursor/rules/) · [`.claude/rules/`](.claude/rules/).
...
| Tool            | Native assets                                                                                                 |
| --------------- | ------------------------------------------------------------------------------------------------------------- |
| **Codex**       | `.codex/rules/` (→ `.agents/rules/`), `.codex/skills/`                                                        |
| **Cursor**      | `.cursor/rules/` (→ `.agents/rules/`)                                                                         |
| **Claude Code** | `.claude/CLAUDE.md`, `.claude/rules/` (→ `.agents/rules/`), `.claude/skills/` ([CLAUDE.md](CLAUDE.md) → here) |
