...
Archive for the **Companion SDK** requirement and planning documents — the first AI app hosted in this `ying-ai` monorepo (chat + Persona + memory + tools, plus an independent Story Mode runtime). Migrated here from the original standalone `ying-companion` repository (now renamed `ying-ai` on GitHub); content below is kept as historical record and is **not** rewritten to match the new repo/package names. AI agents use this directory to understand this app's intent, plan work, and execute staged implementation tasks.

> **AI entry:** [AGENTS.md](../../AGENTS.md) — start there, then [`.requirements/README.md`](../README.md) for the cross-app index, then this README for Companion SDK layout and conventions → read `prompts/` for planning context → read the relevant `stages/{version}/stage-{NN}/` task doc before implementing. Requirement **content is Chinese**; this README is English for agent readability.
...
| [`prompts/05-v1.2-plan.md`](prompts/05-v1.2-plan.md)                       | V1.2 master roadmap      | **V1.2** Web Search tool + Demo AI SDK UI / Sources / docs closure                                                                                                                       |
| [`prompts/06-v1.3-story-mode-plan.md`](prompts/06-v1.3-story-mode-plan.md) | V1.3 master roadmap      | **V1.3 Story Mode**：独立 `story-core` 文字互动叙事（酒馆向）；不塞进 `SimpleChatWorkflow`；种子故事 → 持久化 → Demo 游玩/Debug                                                          |

**Relationship:** `02-execution.md` was used to produce `03-v1.0-plan.md` and constrains Core/SDK releases. Stage task files under `stages/{version}/` are the **detailed specs** — prefer them over high-level bullets inside plan files when implementing.
...
**V1.0 topic:** `@ying-ai/ai-core` core SDK plus `apps/model-runtime-demo` debug workbench. The release covers runtime, DI, chat workflow, memory, emotion, tools, workflow orchestration, and observability. Core must **not** read env vars or host demo UI.
...
**"Where did requirements move from?"** → Original path (in `ying-companion`) `docs/requirements/` → `.requirements/` → now `.requirements/companion/` after migrating into the general-purpose `ying-ai` monorepo. Prefer `.requirements/companion/` in new links and reads.
